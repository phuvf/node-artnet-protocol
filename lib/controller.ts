...
export class ArtNetController extends EventEmitter {
  ...
  constructor(isController: boolean = false) {
    ...
    Object.entries(interfaces).forEach(([ifName, addresses]) => {
      ...
      addresses.forEach((addressInfo) => {
        prefixes[addressInfo.cidr as string] = ip6addr.createCIDR(
          addressInfo.cidr as string
        );
      });
    });
  ...
  public bind(host?: string) {
    ...
    if (host != null) {
      ...
      if (prefixInfo) {
        ...
        throw Error(
          "Bind host " + host + " does not match any network interface"
        );
      }
    } else {
    ...
    if (broadcastAddress !== null) {
      ...
      socketBroadcast.on(
        "listening",
        this.onSocketBroadcastListening.bind(this)
      );
  ...
  public async close() {
    ...
    await Promise.all([
      new Promise((resolve) =>
        this.socketBroadcast?.close(() => resolve(undefined))
      ),
      new Promise((resolve) =>
        this.socketUnicast?.close(() => resolve(undefined))
      ),
    ]);
  }
  ...
  private onSocketMessage(socketType: string, msg: Buffer, rinfo: RemoteInfo) {
    ...
    } else if (packet instanceof ArtTimeCode) {
